...
// than scrolling. This adds a simple toggle + dropdown, matching the dark
// header's own colors rather than the rest of the (light) site.
  ...
  return (
    <div className="md:hidden">
      <button
        ...
        className="rounded-md p-2 text-white/85 hover:text-white"
      >
        {open ? <CloseIcon className="h-5 w-5" /> : <MenuIcon className="h-5 w-5" />}
      </button>

      {open && (
        <div className="absolute inset-x-0 top-full border-t border-white/10 bg-[#0E2A52] px-6 py-4">
          <nav className="flex flex-col gap-1">
            {LINKS.map((link) => (
              <a
                ...
                className="rounded-md px-2 py-2.5 text-sm font-medium text-white/85 hover:text-white"
              >
                {link.label}
              </a>
            ))}
